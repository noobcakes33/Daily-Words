...
class DailyWords:
    def __init__(self):
        ...
        self.when = "05:00"  # In 24 hrs format NOT 12 hrs -- Notice Cloud Server Time 
    ...
    def send_email_with_words(self):
        ...
        for i in range(len(self.receiver_emails)):
            ...
            with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=context) as server:
                ...
                server.sendmail(self.sender_email, self.receiver_emails[i], message.as_string())

    def hold(self, d):
        while True:
            current = strftime("%Y-%m-%d %H:%M:%S", gmtime())
            current = current.split(" ")[1].split(":")
            current_hr, current_min,  current_sec = current[0], current[1], current[2]
            hr = self.when.split(":")[0]
            min = self.when.split(":")[1]
            time.sleep(1)
            print(current_hr, current_min, current_sec)
            if int(current_hr) == int(hr) and int(current_min) == int(min):
                #print("True")
                time.sleep(d)
                break
